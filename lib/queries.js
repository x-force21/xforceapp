...
module.exports = {
    ...
    getUsers: async () => {
        ...
        getProject: async (root, { id }) => {
            ...
            try {
                db = await connectDB ()
                project = await db.collection('projects').find({_id: ObjectId(id)}).toArray();
            } catch (error) {
                console.error(error)
            }        
            return project
        ...
        getInscription: async (root, { codigoProyecto }) => {
            let db 
            let inscription=[]
            try {
                db = await connectDB ()
                inscription = await db.collection('inscription').find({codigoProyecto: codigoProyecto}).toArray();
            } catch (error) {
                console.error(error);
            }        
            return inscription
